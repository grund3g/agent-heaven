...
describe("agent-binaries", () => {
  it("reports found=true when explicit paths are spawnable", async () => {
    ...
    expect(res.gemini.found).toBe(true);
  });
  ...
  it("reports found=false for missing paths", async () => {
    ...
    const missing3 = path.join(os.tmpdir(), `agent-heaven-missing-gemini-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    const settings: any = {
      agents: {
        codex: { path: missing1 },
        claude: { path: missing2 },
        gemini: { path: missing3 }
      }
    };
    ...
    expect(res.gemini.found).toBe(false);
  });
});
